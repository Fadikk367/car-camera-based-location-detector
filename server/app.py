...
def predict():
    ...
    for index, frame in enumerate(frames):
        ...
        for country in extract_data_from_plates(frame):
            data['countries'][country] = data['countries'].get(country, 0) + weights['plate']
        ...
        for country in countries:
            data['countries'][country] = data['countries'].get(country, 0) + weights['route'] * probability
        ...
        for country in countries_from_text.items():
            data['countries'][country[0]] = data['countries'].get(country[0], 0) + weights['plate'] * country[1]
